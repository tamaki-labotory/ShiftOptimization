...
第二段階:従業員が勤務するかどうかを決定(超過人数最小化),勤務希望も次点で最大化
"""
import sys
import time
...
def solve(file_path,printLog):
    ...
    for p in range(n_L):
        ...
        model.addCons(sum(u[s] for s in range(n_S)) <= 100)
        model.addCons(
    ...
    model.setObjective(
        sum(
            ...
            ) - 0.00001 * sum(w[s][t] * v[p][s] * h_P[p][t] for s in range(n_S) for t in range(n_T) for p in range(n_L)),
