...
class ShiftScheduler7(ShiftScheduler):
    def solve(self):
        ...
        if not os.path.exists(u_values_file):
            ...
            for l in range(self.n_L):
                model.addCons(sum(u[l][s] for s in range(self.n_S)) == a)
            for t in range(self.n_T):
                model.addCons(sum(u[l][s]*self.w[s][t] for s in range(self.n_S) for l in range(self.n_L)) >= z)
            
            model.setObjective(
                omega_1*phi_1*sum(self.w[s][t]*u[l][s]*self.h_P[l][t] for s in range(self.n_S) for t in range(self.n_T) for l in range(self.n_L))
                - omega_2 * phi_2 * sum(self.w[s][t]*u[l][s]*self.h_N[l][t] for s in range(self.n_S) for t in range(self.n_T) for l in range(self.n_L))
                +z,
                "maximize"
            )
            ...
            for l in range(self.n_L):
                ...
                for t in range(self.n_T):
                    model.addCons(sum(u[s] * self.w[s][t] for s in range(self.n_S)) >= z)
                
                # 目的関数の設定
                model.setObjective(
                    omega_1 * phi_1 * sum(
                        self.w[s][t] * u[s] * self.h_P[l][t] for s in range(self.n_S) for t in range(self.n_T)
                    )
                    - omega_2 * phi_2 * sum(
                        self.w[s][t] * u[s] * self.h_N[l][t] for s in range(self.n_S) for t in range(self.n_T)
                    )
                    + z,
                    "maximize"
                )
