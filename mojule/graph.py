...
class ShiftDataVisualizer:
    ...
    def show_shift(self):
        ...
        fig.tight_layout(pad=6.0)
        # 2. シフトパターンのヒートマップ
        shift_img = axs.imshow(self.data["shift_patterns"], cmap="Blues", aspect="auto")
        axs.set_title("Shift Patterns", fontsize=14, fontweight="bold")
        axs.set_xlabel("Time Slot", fontsize=12)
        axs.set_ylabel("Pattern ID", fontsize=12)

        axs.set_xticks(np.arange(np.array(self.data["shift_patterns"]).shape[1]), minor=False)  # Major ticks at integers
        axs.set_yticks(np.arange(np.array(self.data["shift_patterns"]).shape[0]), minor=False)
        axs.set_xticks(np.arange(-0.5, np.array(self.data["shift_patterns"]).shape[1]), minor=True)  # Minor ticks for grid lines
        axs.set_yticks(np.arange(-0.5, np.array(self.data["shift_patterns"]).shape[0]), minor=True)
        axs.set_xticklabels(np.arange(1, np.array(self.data["shift_patterns"]).shape[1] + 1))  # 整数ラベルのみ
        axs.set_yticklabels(np.arange(1, np.array(self.data["shift_patterns"]).shape[0] + 1))
    ...
    def show_number_of_labor(self):
        ...
        # 1. 必要な従業員数のグラフ
        axs.bar(range(len(self.data["required_employees"])), self.data["required_employees"], color="skyblue", edgecolor="black")
        axs.set_title("Required Employees", fontsize=14, fontweight="bold")
        axs.set_xlabel("Time Slot", fontsize=12)
        axs.set_ylabel("Required Number of Employees", fontsize=12)
        axs.grid(axis="y", linestyle="--", alpha=0.7)
        axs.set_xticks(np.arange(len(np.array(self.data["required_employees"]))), minor=False)  # Major ticks at integers
        axs.set_xticks(np.arange(-0.5, len(np.array(self.data["required_employees"]))), minor=True)  # Minor ticks for grid lines
        axs.set_xticklabels(np.arange(1, len(np.array(self.data["required_employees"])) + 1))  # 整数ラベルのみ
        axs.set_xlim(-0.5,len(np.array(self.data["required_employees"]))-0.5)
    ...
    def show_prefere(self):
        ...
        # 3. 従業員の希望と利用不可スロットのオーバーレイ
        pref_img = axs.imshow(self.data["preferences"], cmap=ListedColormap(["white", "Green"]), aspect="auto", alpha=0.5)
        unavail_img = axs.imshow(self.data["unavailable_slots"], cmap=ListedColormap(["white", "Red"]), aspect="auto", alpha=0.5)
        ...
        axs.set_xticks(np.arange(np.array(self.data["unavailable_slots"]).shape[1]), minor=False)  # Major ticks at integers
        axs.set_yticks(np.arange(np.array(self.data["unavailable_slots"]).shape[0]), minor=False)
        axs.set_xticks(np.arange(-0.5, np.array(self.data["unavailable_slots"]).shape[1]), minor=True)  # Minor ticks for grid lines
        axs.set_yticks(np.arange(-0.5, np.array(self.data["unavailable_slots"]).shape[0]), minor=True)
        axs.set_xticklabels(np.arange(1, np.array(self.data["unavailable_slots"]).shape[1] + 1))  # 整数ラベルのみ
        axs.set_yticklabels(np.arange(1, np.array(self.data["unavailable_slots"]).shape[0] + 1))
        axs.grid(which="both", color="gray", linestyle="--", linewidth=0.5)
        axs.grid(which="major", color="none")
        # レジェンドの作成
        from matplotlib.lines import Line2D
